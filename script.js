...
// CANVAS BACKGROUND (particles + hex + scanline)
...
function randomHexGroup() { return Array.from({length:4},()=>hexChars[Math.floor(Math.random()*16)]+hexChars[Math.floor(Math.random()*16)]).join(' '); }
...
function animate(){
  ...
  particles.forEach(p=>{
    ...
    if(isScan){ ctx.save(); ctx.shadowColor='rgba(0,242,255,0.35)'; ctx.shadowBlur=12; ctx.fillText(p.text,p.x,p.y); ctx.restore(); }
    else ctx.fillText(p.text,p.x,p.y);
    p.y+=p.speed; if(p.y>canvas.height){ p.y=0; p.text=randomHexGroup(); }
  });
  // scan line
  ctx.save(); ctx.strokeStyle='rgba(0,242,255,0.1)'; ctx.lineWidth=2; ctx.shadowColor='rgba(0,242,255,0.18)'; ctx.shadowBlur=18;
  ctx.beginPath(); ctx.moveTo(0,scanY); ctx.lineTo(canvas.width,scanY); ctx.stroke(); ctx.restore();
  scanY+=scanSpeed; if(scanY>canvas.height+14) scanY=-14;
